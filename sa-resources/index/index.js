	...
	methods: {
		...
		right_copy: function () {
			this.showTab({
				...
				url: this.getTabUrl(this.rightTab)
			});
		},
		// 右键 悬浮 
		right_xf: function () {
		...
		// 右键 - 关闭左侧 
		right_close_left: function () {
			// 先滑到最左边 
			this.scrollX = 0;
			// 获取白名单
			var i = 0;
			var length = this.tabList.indexOf(this.rightTab);
			var whiteList = [this.homeTab.id + '', this.rightTab.id];
			// for (var j = length; j < this.tabList.length; j++) {
			// 	whiteList.push(this.tabList[j].id + '');
			// }
			// 递归删除
			var deleteFn = function () {
				// 如果已经遍历全部 
		...
		// 右键 - 关闭右侧 
		right_close_right: function () {
			// 先滑到最左边 
			this.scrollX = 0;
			// 获取白名单(关闭右侧，左侧为白名单)
			var i = this.tabList.indexOf(this.rightTab);
		...
		addTab: function (tab) {
			...
			if (this.tabList.length > 20 && this.tabList.length < 30) {
				sa_admin.$message({
					message: '选项卡过多会造成窗口卡顿，建议您关闭不使用的窗口',
					type: 'warning'
				});
			}
		},
