...
(function () {
	...
	sa.ajax = function (url, data, success200, cfg) {
		...
		var defaultCfg = {
			...
			success0: success200, // code=200, 代表成功 
			...
			error: function (xhr, type, errorThrown) { // ajax发生异常时的默认处理函数
				if (xhr.status == 0) {
		...
		let options = {
			...
			error: function (xhr, type, errorThrown) {
				setTimeout(function () {
					sa.hideLoading();
					return cfg.error(xhr, type, errorThrown);
				}, cfg.sleep);
			},
	...
	sa.urlEncode = function (param, key, encode) {
		let that = this;
		if (param == null) return '';
		let paramArray = [];
		let t = typeof (param);
		if (t == 'string' || t == 'number' || t == 'boolean') {
			paramArray.push(key + '=' + ((encode == null || encode) ? encodeURIComponent(param) : param));
		} else {
			for (let i in param) {
				let k = key == null ? i : key + (param instanceof Array ? '[' + i + ']' : '.' + i);
				paramArray.push(that.urlEncode(param[i], k, encode));
			}
		}
		return paramArray.join('&');
	}

	sa.get = function (url, data, success, fail, error, complete) {
		let that = this;
		if (url.indexOf("?") >= 0) {
			url = url + "&" + that.urlEncode(data);
		} else {
			url = url + "?" + that.urlEncode(data);
		}
	...
	sa.post = function (url, data, success, fail, error, complete) {
		this.ajax(url, JSON.stringify(data), success, {
			type: 'post',
			fail: fail,
			error: error,
			complete: complete
...
(function () {
	...
	me.showIframe = function (title, url, w, h, shadeClose, end) {
		// 参数修正
		w = w || '95%';
		h = h || '95%';
	...
	me.showIframe2 = function (title, url, evalStr, w, h, end) {
		// 参数修正
		w = w || '95%';
		h = h || '95%';
		// 弹出面板 
